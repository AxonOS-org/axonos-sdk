...
impl CapabilitySet {
    ...
    ///
    /// Returns a new set containing capabilities present in **either** input.
    ///
    /// # WCET
    /// 1 cycle (bitwise OR).
    ///
    ...
    /// let both = nav.union(quality);
    /// assert!(both.contains(Capability::Navigation));
    /// assert!(both.contains(Capability::SessionQuality));
    /// ```
    #[must_use]
    ...
    /// Intersection with another capability set.
    ///
    /// Returns a new set containing capabilities present in **both** inputs.
    ///
    /// # WCET
    /// 1 cycle (bitwise AND).
    #[must_use]
    ...
    /// Set difference: capabilities in `self` not in `other`.
    ///
    /// # WCET
    /// 2 cycles (NOT + AND).
    #[must_use]
    ...
    ///
    /// A subset relation is reflexive: every set is a subset of itself.
    ///
    /// # WCET
    /// 2 cycles (NOT + AND + compare zero).
    ///
    /// # Example
    ///
    /// ```
    /// use axonos_sdk::{Capability, CapabilitySet};
    /// let basic = CapabilitySet::new().with(Capability::Navigation);
    /// let full = basic
    ...
    /// True if `self` and `other` share no capabilities.
    ///
    /// # WCET
    /// 2 cycles (AND + compare zero).
    #[must_use]
...
mod tests {
    ...
    fn union_with_self_is_idempotent() {
        let s = CapabilitySet::new().with(Capability::Navigation);
        assert_eq!(s.union(s).as_raw(), s.as_raw());
    }

    #[test]
    fn intersection_only_common() {
        ...
        assert!(common.contains(Capability::SessionQuality));
        assert!(!common.contains(Capability::Navigation));
        assert!(!common.contains(Capability::WorkloadAdvisory));
        assert_eq!(common.len(), 1);
    }

    #[test]
    fn intersection_disjoint_is_empty() {
        let a = CapabilitySet::new().with(Capability::Navigation);
        let b = CapabilitySet::new().with(Capability::SessionQuality);
    ...
    fn difference_removes_other() {
        ...
        assert!(only_a.contains(Capability::Navigation));
        assert!(!only_a.contains(Capability::SessionQuality));
    }
    ...
    fn is_subset_empty_is_subset_of_any() {
        ...
        let any = CapabilitySet::new().with(Capability::Navigation);
